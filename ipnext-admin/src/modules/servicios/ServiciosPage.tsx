...
function makeKpis(servicios: Servicio[], gastoLabel: string) {
  const total = servicios.reduce((a, s) => a + s.montoMensual, 0)
  const activos = servicios.filter((s) => s.estado === 'activo').length
  const proximos = servicios.filter((s) => s.estado === 'proximo_vencer').length
  return [
    { icon: <DollarSign size={20} className="text-[#E42313]" />, label: gastoLabel, value: `$${total.toLocaleString('es-AR')}`, iconBg: 'bg-red-50' },
...
const internetCols = [
  ...
  { key: 'monto', header: 'Monto Mensual', render: (s: Servicio) => <span className="font-semibold">${s.montoMensual.toLocaleString('es-AR')}</span> },
...
const seguridadCols = [
  ...
  { key: 'monto', header: 'Costo Mensual', render: (s: Servicio) => <span className="font-semibold">${s.montoMensual.toLocaleString('es-AR')}</span> },
      ...
      <Card className="p-0 overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'seguridad' && <ServiciosTable servicios={seguridad} kpis={[
            { icon: <DollarSign size={20} className="text-[#E42313]" />, label: 'Gasto del mes', value: `$${seguridad.reduce((a, s) => a + s.montoMensual, 0).toLocaleString('es-AR')}`, iconBg: 'bg-red-50' },
          ...
          {activeTab === 'software' && <ServiciosTable servicios={software} kpis={[
            { icon: <DollarSign size={20} className="text-[#E42313]" />, label: 'Gasto del mes', value: `$${software.reduce((a, s) => a + s.montoMensual, 0).toLocaleString('es-AR')}`, iconBg: 'bg-red-50' },
