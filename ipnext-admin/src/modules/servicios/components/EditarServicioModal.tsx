...
import type { Servicio, ServicioTipo } from '@/types/servicio.types'
...
export function EditarServicioModal({ open, onClose, servicio }: Props) {
  ...
  useEffect(() => {
    if (servicio) setForm({
      ...
      costoMensual: String(servicio.costoMensual),
      vtoFactura: servicio.vtoFactura ?? '',
      renovacion: servicio.renovacion ?? '',
    })
  }, [servicio])

  const mutation = useMutation({
    mutationFn: () => api.patch(`/servicios/item/${servicio?.id}`, {
      nombre: form.nombre,
      tipo: form.tipo,
  ...
  return (
    <Modal open={open} onClose={onClose} title="Editar Servicio" size="md">
